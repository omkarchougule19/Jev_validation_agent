...
def test_race_streams_both_lanes_to_the_end():
    events = _events(lambda c: ("pass", 5.0), lambda c: ("block", 9.0))
...
def test_rate_limit_stops_only_the_groq_lane():
    ...
    events = _events(lambda c: ("pass", 5.0), groq)
...
def test_provider_error_is_reported_not_raised():
    ...
    events = _events(jev, lambda c: ("pass", 5.0))
    assert any(e["type"] == "error" and e["lane"] == "jev" for e in events)


def test_second_race_within_a_minute_is_blocked():
    _events(lambda c: ("pass", 1.0), lambda c: ("pass", 1.0))
    events = _events(lambda c: ("pass", 1.0), lambda c: ("pass", 1.0))
...
def test_daily_cap_blocks_with_limit_reason(monkeypatch):
    ...
    assert _events(lambda c: ("pass", 1.0), lambda c: ("pass", 1.0)) == [{"type": "blocked", "reason": "limit"}]


def test_random_race_picks_race_size_cases():
    pool = CASES * 20
    events = _events(lambda c: ("pass", 1.0), lambda c: ("pass", 1.0), cases=None) if False else \
        [json.loads(s.removeprefix("data: ")) for s in race.race_events(lambda c: ("pass", 1.0), lambda c: ("pass", 1.0), all_cases=pool)]
